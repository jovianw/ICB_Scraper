from scrapers.common.scraper import AbstractScraper
...
class Scraper(AbstractScraper):
    def __init__(self, patience=10, webdriverOptions=None):
        ...
        self.name = "NYTimes"
        self.searchLink = "https://api.nytimes.com/svc/search/v2/articlesearch.json"

    def getJSON(self, keyPhrase, numLinks):
        if not os.getenv('NYTimes_Key'):
        ...
        for page in range(pagesToGet):
            ...
            params = {'q': keyPhrase, 'api-key': os.getenv('NYTimes_Key'), 'type_of_material.contains': 'Text', 'page': page}
    ...
    def getLinks(self, keyPhrase, numLinks):
        ...
        for page in range(pagesToGet):
            logging.root.info(f"NYTimes: Getting new page...")
            params = {'q': keyPhrase, 'api-key': os.getenv('NYTimes_Key'), 'type_of_material.contains': 'Text', 'page': page, 'fl': 'web_url'}
            response = requests.get(url, params=params)
            if response.json()['status'] != 'OK': # If status is not OK
                logging.root.warning(response.json()['errors'][0])
                continue
